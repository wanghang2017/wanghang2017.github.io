import { networks, crypto, address, payments, Transaction as Transaction$1, Psbt, script, initEccLib } from 'bitcoinjs-lib';
import bs58check, { encode, decode } from 'bs58check';
...
const bip32 = /*#__PURE__*/BIP32Factory(ecc);
const pathMap = {
  ...
  switch (scriptType) {
    ...
    case ScriptType.P2TR:
      const result = await snap.request({
        ...
        params: {
          type: 'confirmation',
          content: panel([heading('Access your extended public key'), text(`Do you want to allow ${origin} to access Bitcoin ${networkName} ${scriptType} extended public key?`)])
        }
      });
      if (result) {
        const {
...
async function getAllXpubs(origin, snap) {
  const result = await snap.request({
    ...
    params: {
      type: 'confirmation',
      content: panel([heading('Access your extended public key'), text(`${origin} is trying to access your Bitcoin Legacy, SegWit, TapRoot and Native SegWit extended public keys.`)])
    }
  });
  try {
    if (result) {
...
class AccountSigner {
  ...
  getTapRootSinger(path = '0/0') {
    ...
    const tweakedSinger = tapAccountSinger.node.tweak(crypto.taggedHash('TapTweak', tapAccountSinger.node.publicKey.slice(1)));
...
class PsbtHelper {
  ...
  get inputAmount() {
    return this.tx.data.inputs.reduce((total, input, index) => {
      ...
      if (input.nonWitnessUtxo) {
        const prevTx = Transaction$1.fromHex(input.nonWitnessUtxo.toString('hex'));
  ...
  get fromAddresses() {
    return this.tx.data.inputs.map((input, index) => {
      if (input.nonWitnessUtxo) {
        const prevOuts = Transaction$1.fromHex(input.nonWitnessUtxo.toString('hex')).outs;
        const vout = this.tx.txInputs[index].index;
...
class Transaction {
  ...
  isDefinedSignType(signType) {
    return signType === Transaction$1.SIGHASH_DEFAULT || script.isDefinedHashType(signType);
  }
...
async function signPsbt(domain, snap, psbt, network, scriptType, signInputIndex, signType) {
  ...
  const result = await snap.request({
    ...
    params: {
      type: 'confirmation',
      content: panel([heading('Sign Bitcoin Transaction'), text(`Please verify this ongoing Transaction from ${domain}`), divider(), panel(Object.entries(txDetails).map(([key, value]) => text(`**${key}**:\n ${value}`)))])
    }
  });
  if (result) {
    try {
...
async function getMasterFingerprint(snap) {
  const {
    mfp
  } = await getHDRootNode(snap, networks.bitcoin);
  return mfp;
}
  ...
  switch (action) {
    ...
    case 'set':
      const result = await snap.request({
        ...
        params: {
          type: 'confirmation',
          content: panel([heading('Switch your network'), text(`Do you want to allow ${origin} to switch Bitcoin network to ${target}?`)])
        }
      });
      if (result) {
  ...
  switch (key) {
    ...
    case KeyOptions.Credential:
      ...
      const result = await snap.request({
        ...
        params: {
          type: 'confirmation',
          content: panel([heading(param.prompt), text(param.description)])
        }
      });
      if (result) {
        const lightning = await getPersistedData(snap, 'lightning', {});
...
async function signLNInvoice(domain, snap, invoice) {
  ...
  const result = await snap.request({
    ...
    params: {
      type: 'confirmation',
      content: panel([heading('Sign Lightning Transaction'), text(`Please verify this ongoing transaction from ${domain}`), divider(), panel(Object.entries(invoiceContent).map(([key, value]) => text(`**${key}**:\n ${value}`)))])
    }
  });
  if (result) {
...
async function getSimpleAddress(origin, snap, network) {
  ...
  const result = await snap.request({
    ...
    params: {
      type: 'confirmation',
      content: panel([heading('Access your account addresses'), text(`Do you want to allow ${origin} to access Bitcoin ${networkName} addresses?`)])
    }
  });
  if (result) {
    const {
...
export { onRpcRequest };
//# sourceMappingURL=btcsnap.esm.js.map
